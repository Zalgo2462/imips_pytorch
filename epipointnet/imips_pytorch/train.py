...
class ImipsTrainer:
    ...
    @staticmethod
    def _find_correspondences(pair: epipointnet.data.pairs.CorrespondencePair,
    ...
    @staticmethod
    def _create_correspondence_image(pair: epipointnet.data.pairs.CorrespondencePair, image_1_pixels_xy: torch.Tensor,
    ...
    def _image_to_patch_batch(image_np: np.ndarray, keypoints_xy: torch.Tensor, diameter: int) -> torch.Tensor:
        ...
        image = epipointnet.datasets.image.load_image_for_torch(image_np)
        radius = (diameter - 1) // 2
    ...
    def _train_pair(self, pair: epipointnet.data.pairs.CorrespondencePair, iteration: int) -> torch.Tensor:
        ...
        img_1 = epipointnet.datasets.image.load_image_for_torch(pair.image_1)
        img_2 = epipointnet.datasets.image.load_image_for_torch(pair.image_2)
