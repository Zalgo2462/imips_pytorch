...
class OHNMImipTrainer:
    ...
    def _train_pair(self, pair: epipointnet.data.pairs.CorrespondencePair,
        ...
        img_1_correspondences, img_1_correspondences_mask = self.find_correspondences(
            pair, img_2_kp_candidates[:, :, 0], inverse=True,
        ...
        img_2_correspondences, img_2_correspondences_mask = self.find_correspondences(
            pair, img_1_kp_candidates[:, :, 0], inverse=False,
            exclude_border_px=(self._network.receptive_field_diameter() - 1) // 2
        )

        (img_1_kp_candidates, img_1_inlier_channels_by_max, img_1_outlier_channels_by_max,
         img_1_inlier_channels_by_top_k, img_1_outlier_channels_by_top_k) = self.sort_candidates_and_generate_labels(
        ...
        (img_2_kp_candidates, img_2_inlier_channels_by_max, img_2_outlier_channels_by_max,
         img_2_inlier_channels_by_top_k, img_2_outlier_channels_by_top_k) = self.sort_candidates_and_generate_labels(
        ...
        t_board_writer.add_scalar("training/apparent inliers (top k)", apparent_inliers_top_k, iteration)
        t_board_writer.add_scalar("training/apparent outliers (top k)", apparent_outliers_top_k, iteration)
        ...
        corr_patches[img_1_correspondences_mask, :, :, :] = OHNMImipTrainer.image_to_patch_batch(
            img_1, img_1_correspondences[:, img_1_correspondences_mask], patch_diameter
        )

        self._train_patches(
            kp_patches, corr_patches, img_1_inlier_channels_by_top_k, img_1_outlier_channels_by_top_k
        )
        ...
        corr_patches[img_2_correspondences_mask, :, :, :] = OHNMImipTrainer.image_to_patch_batch(
            img_2, img_2_correspondences[:, img_2_correspondences_mask], patch_diameter
        )

        loss_logs = self._train_patches(
            kp_patches, corr_patches, img_2_inlier_channels_by_top_k, img_2_outlier_channels_by_top_k
        )
    ...
    def count_inliers(pair: epipointnet.data.pairs.CorrespondencePair,
        ...
        (img_1_kp_candidates, img_1_inlier_channels_by_max, img_1_outlier_channels_by_max,
         img_1_inlier_channels_by_top_k, img_1_outlier_channels_by_top_k) = \
            OHNMImipTrainer.sort_candidates_and_generate_labels(
        ...
        num_apparent_inliers_by_top_k = (img_1_inlier_channels_by_top_k & img_2_inlier_channels_by_top_k).sum()
    ...
    def _evaluate_dataset(self, dataset: torch.utils.data.Dataset) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        total_inliers_by_top_k = torch.tensor([0], device=self._device)
        for pair in dataset:
            ...
            num_apparent_inliers, num_true_inliers, num_inliers_by_top_k = OHNMImipTrainer.count_inliers(
            ...
            total_inliers_by_top_k = total_inliers_by_top_k + num_inliers_by_top_k
        return (total_apparent_inliers / num_samples, total_true_inliers / num_samples,
                total_inliers_by_top_k / num_samples)
    ...
    def train(self, iterations: int, eval_frequency: int):
        ...
        for iteration in range(1, iterations + 1):
            ...
            if iteration % eval_frequency == 0:
                train_apparent_inliers_avg, train_true_inliers_avg, train_inliers_by_top_k_avg = \
                    self._evaluate_dataset(self._train_eval_dataset)
                eval_apparent_inliers_avg, eval_true_inliers_avg, eval_inliers_by_top_k_avg = \
                ...
                t_board_writer.add_scalar(
                    "training_evaluation/apparent inliers (top k)", train_inliers_by_top_k_avg, iteration
                )
                ...
                t_board_writer.add_scalar(
                    "evaluation/apparent inliers (top k)", eval_inliers_by_top_k_avg, iteration
                )
                save_info = {
                    ...
                    "training_evaluation/apparent inliers (top k)": train_inliers_by_top_k_avg,
                    "evaluation/apparent inliers": eval_apparent_inliers_avg,
                    "evaluation/true inliers": eval_true_inliers_avg,
                    "evaluation/apparent inliers (top k)": eval_inliers_by_top_k_avg,
                    "seed": self._seed,
                }
