...
class HalfSimpleConvA(imips.ImipNet):

    def __init__(self, num_convolutions: int = 14, input_channels: int = 1, output_channels: int = 64,
                 bias: bool = True):
        imips.ImipNet.__init__(self, input_channels, output_channels)
        self._simple_conv = SimpleConv(num_convolutions, input_channels, 2 * output_channels, bias)

        self._final_conv = torch.nn.Conv2d(2 * output_channels, output_channels, kernel_size=1, bias=False)
        with torch.no_grad():
            self._final_conv.weight.fill_(0)
            for filter_idx in range(64):
                for chan_idx in [filter_idx + 64 * x for x in range(2)]:
                    self._final_conv.weight[filter_idx, chan_idx, :, :] = 0.5
...
class HalfSimpleConvB(imips.ImipNet):
    ...
    def __init__(self, num_convolutions: int = 14, input_channels: int = 1, output_channels: int = 64,
        ...
        self._final_conv = torch.nn.Conv2d(2 * output_channels, output_channels, kernel_size=1, bias=False)
        torch.nn.init.xavier_uniform_(self._final_conv.weight, gain=torch.nn.init.calculate_gain('leaky_relu', 0.02))
...
class HalfSimpleConvC(imips.ImipNet):
    ...
    def __init__(self, num_convolutions: int = 14, input_channels: int = 1, output_channels: int = 64,
        ...
        layers_list = []
        layers_list.extend([
            torch.nn.Conv2d(input_channels, output_channels, kernel_size=3, bias=bias),
            torch.nn.LeakyReLU(negative_slope=0.02)  # default for tensorflow/ imips
        ])

        for i in range(num_convolutions - 2):
            layers_list.extend([
                torch.nn.Conv2d(output_channels, output_channels, kernel_size=3, bias=bias),
                torch.nn.LeakyReLU(negative_slope=0.02)
            ])
...
class SimpleConvNoNorm(imips.ImipNet):
