...
class ScalePyramid(torch.nn.Module):
    def __init__(self,
                 max_octaves: Optional[int] = None,  # Added max_octaves for performance to Kornia ScalePyramid
                 n_levels: int = 3,
    ...
    def __repr__(self) -> str:
        return self.__class__.__name__ + \
               '(max_octaves=' + str(self.max_octaves) + ', ' + \
    ...
    def forward(self, x: torch.Tensor) -> Tuple[List, List, List]:
        ...
        while True:
            ...
            next_octave_first_level = torch.nn.functional.interpolate(pyr[-1][-1].squeeze(1), scale_factor=0.5,
                                                                      mode='bilinear',
                                                                      align_corners=False)
            pixel_distance *= 2.0
            cur_sigma = self.init_sigma
            if (min(next_octave_first_level.size(2),
                    next_octave_first_level.size(3)) <= self.min_size) or oct_idx + 1 == self.max_octaves:
                break
            pyr.append([next_octave_first_level.unsqueeze(1)])
