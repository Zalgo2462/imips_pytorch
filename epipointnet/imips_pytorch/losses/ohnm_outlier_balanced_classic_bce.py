...
class OHNMClassicImipBCELoss(ImipLoss):
    ...
    def forward_with_log_data(self, maximizer_outputs: torch.Tensor, correspondence_outputs: torch.Tensor,
        ...
        if aligned_maxima_bce_labels.numel() == 0:
            ...
            weights = torch.ones_like(aligned_maxima_bce_labels)
            weights[~(aligned_maxima_bce_labels.to(dtype=torch.bool))] = self._get_bce_maxima_outlier_weight(
                num_patches_per_channel)

            aligned_maxima_losses = torch.nn.functional.binary_cross_entropy_with_logits(
                maximizer_outputs[maxima_has_data_index_2d], aligned_maxima_bce_labels, weights, reduction="sum"
            )
