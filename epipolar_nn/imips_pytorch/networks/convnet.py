...
class SimpleConv(imips.ImipsNet):
    ...
    def __init__(self, num_convolutions: int = 14, input_channels: int = 1, out_channels: int = 128):
        imips.ImipsNet.__init__(self, input_channels)

        self._num_convolutions = num_convolutions
        self._receptive_field_diameter = num_convolutions * 2 + 1

        num_channels_first_half = out_channels // 2
        ...
        layers_list.append(
            torch.nn.Conv2d(num_channels_first_half, out_channels, kernel_size=3),
        )
        layers_list.append(
