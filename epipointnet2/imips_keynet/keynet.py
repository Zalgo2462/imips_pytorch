...
class UNet(pl.LightningModule):
    def __init__(self, hparams):
        ...
        class up(nn.Module):
            ...
            def forward(self, x1, x2):
                x1 = self.up(x1)
                # assuming all shape inconsistencies come from maxpool and upsample, we should crop the ends of x2
        ...
        size = int(6 * sigma + 1)
        label_mask = get_gaussian_kernel2d((size, size), (sigma, sigma))
    ...
    def derive_loss(self, image_1_logits: torch.Tensor, image_1_keypoints_xy: torch.Tensor,
                    ...
                    scale: float) -> Tuple[torch.Tensor, Dict[str, torch.Tensor], torch.Tensor, torch.Tensor,
        ...
        image_1_outliers = ~image_1_inliers  # image_1_inlier_distance_mask & image_2_corrs_mask
        ...
        image_2_outliers = ~image_2_inliers  # image_2_inlier_distance_mask & image_1_corrs_mask
        ...
        image_1_label_map, image_1_lm_weights, image_1_outlier_mask, image_1_decorr_mask = self.draw_label_maps(
            image_1_logits, image_1_keypoints_xy, image_1_keypoints_xy, image_1_inliers, image_1_outliers)
        # NOTE: we pass image_corrs_xy in as the inlier points to get the network to train on a true correspondence
        image_2_label_map, image_2_lm_weights, image_2_outlier_mask, image_2_decorr_mask = self.draw_label_maps(
            image_2_logits, image_1_corrs_xy, image_2_keypoints_xy, image_2_inliers, image_2_outliers)
        ...
        image_1_decorr_loss *= 50
        image_2_decorr_loss *= 50

        # aggregate the losses
        image_1_loss = image_1_anchor_loss + image_1_outlier_loss + image_1_decorr_loss
        image_2_loss = image_2_anchor_loss + image_2_outlier_loss + image_2_decorr_loss
        ...
        loss_dict = {
            'loss': loss,
            "apparent_inliers": total_apparent_inliers.mean(),
            "true_inliers": total_true_inliers.mean(),
            "outliers": total_outliers.mean(),
            'image_1_loss': image_1_loss,
            'image_1_anchor_loss': image_1_anchor_loss,
            'image_1_outlier_loss': image_1_outlier_loss,
            'image_1_decorr_loss': image_1_decorr_loss,
            'image_2_loss': image_2_loss,
            'image_2_anchor_loss': image_2_anchor_loss,
            'image_2_outlier_loss': image_2_outlier_loss,
            'image_2_decorr_loss': image_2_decorr_loss,
        }

        return loss, loss_dict, image_1_inliers, image_2_inliers, image_1_outliers, image_2_outliers

    def draw_label_maps(self, logits: torch.Tensor, inliers_xy: torch.Tensor, outliers_xy: torch.Tensor,
                        inliers: torch.Tensor, outliers: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor,
                                                                                torch.Tensor, torch.Tensor]:
        image_br = torch.tensor(logits.shape[2:4],
                                device=self.device,
                                dtype=torch.long).flip(0).view(1, 2, 1)  # .expand_as(inliers_xy)
        px_per_channel = torch.tensor(logits.shape[2] * logits.shape[3], dtype=self.dtype, device=self.device)
        num_inliers = torch.sum(inliers).to(dtype=self.dtype)
        ...
        in_ul = (torch.round(inliers_xy) - self.label_mask.shape[0] // 2).to(dtype=torch.long)
        in_br = (torch.round(inliers_xy) + self.label_mask.shape[0] // 2 + 1).to(dtype=torch.long)

        in_mask_from_ul = (-in_ul).clamp_min(0)
        in_mask_from_br = torch.min(in_br, image_br.expand_as(in_br)) - in_ul

        in_mask_to_ul = in_ul.clamp_min_(0)
        in_mask_to_br = torch.min(in_br, image_br.expand_as(in_br))

        out_ul = (torch.round(outliers_xy) - self.label_mask.shape[0] // 2).to(dtype=torch.long)
        out_br = (torch.round(outliers_xy) + self.label_mask.shape[0] // 2 + 1).to(dtype=torch.long)

        out_mask_from_ul = (-out_ul).clamp_min(0)
        out_mask_from_br = torch.min(out_br, image_br.expand_as(out_br)) - out_ul

        out_mask_to_ul = out_ul.clamp_min_(0)
        out_mask_to_br = torch.min(out_br, image_br.expand_as(out_br))

        for b in range(logits.shape[0]):
            for c in range(logits.shape[1]):
                if inliers[b, c]:
                    # draw label map on top of inlier
                    label_map[b, c, in_mask_to_ul[b, 1, c]:in_mask_to_br[b, 1, c],
                    in_mask_to_ul[b, 0, c]:in_mask_to_br[b, 0, c]] = \
                        self.label_mask[in_mask_from_ul[b, 1, c]:in_mask_from_br[b, 1, c],
                        in_mask_from_ul[b, 0, c]:in_mask_from_br[b, 0, c]]

                    # find the weighting such that weight_fg * px_fg = pos_weight * (weight_bg * px_bg)
                    # and sum weights = 1
                    pos_weight = 10
                    num_positive = self.label_mask[in_mask_from_ul[b, 1, c]:in_mask_from_br[b, 1, c],
                                   in_mask_from_ul[b, 0, c]:in_mask_from_br[b, 0, c]].numel()
                    ...
                    in_mask_to_ul[b, 0, c]:in_mask_to_br[b, 0, c]] |= \
                        self.binary_label_mask[in_mask_from_ul[b, 1, c]:in_mask_from_br[b, 1, c],
                        in_mask_from_ul[b, 0, c]:in_mask_from_br[b, 0, c]]
                if outliers[b, c]:
                    # draw outlier mask on top of outlier
                    outlier_mask[b, c, out_mask_to_ul[b, 1, c]:out_mask_to_br[b, 1, c],
                    out_mask_to_ul[b, 0, c]:out_mask_to_br[b, 0, c]] |= \
                        self.binary_label_mask[out_mask_from_ul[b, 1, c]:out_mask_from_br[b, 1, c],
    ...
    def training_step(self, batch, batch_nb):
        ...
        image_2_tensors = gaussian_blur2d(image_2_tensors, (5, 5), (2, 2))
        ...
        # generate a loss
        loss, loss_dict, _, _, _, _ = self.derive_loss(image_1_logits, image_1_keypoints_xy,
                                                       image_2_logits, image_2_keypoints_xy,
                                                       correspondence_funcs, self.train_scale)

        tensorboard_logs = {"train/" + key: loss_dict[key] for key in loss_dict}
    ...
    def validation_epoch_end(self, outputs):
        return {
            ...
            "val_apparent_inliers": torch.stack([x['apparent_inliers'] for x in outputs]).mean(),
            "val_true_inliers": torch.stack([x['true_inliers'] for x in outputs]).mean(),
            'log': {"val/" + key: torch.stack([x[key] for x in outputs]).mean() for key in outputs[0]}
        }
    ...
    def test_epoch_end(self, outputs):
        return {
            ...
            "test_apparent_inliers": torch.stack([x['apparent_inliers'] for x in outputs]).mean(),
            "test_true_inliers": torch.stack([x['true_inliers'] for x in outputs]).mean(),
            'log': {"test/" + key: torch.stack([x[key] for x in outputs]).mean() for key in outputs[0]}
        }
